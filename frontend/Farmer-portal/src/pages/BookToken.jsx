...
import UberTractorPool from '../components/UberTractorPool';
...
export default function BookToken() {
  ...
  const [step, setStep] = useState(0),
    ...
    [poolDriverDetails, setPoolDriverDetails] = useState(null);

  React.useEffect(() => {
    if (user?.phone_number && !activeBooking) {
      syncActiveBooking(user.phone_number);
    }
  }, [user?.phone_number, activeBooking]);
  ...
  async function book(e) {
    ...
    try {
      ...
      const payload = {
        ...
        ...(transportMode === 'POOL' && poolDriverDetails ? poolDriverDetails : {})
      };
  ...
  <>
    <div className="mt-6" style={{ gridColumn: '1 / -1' }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4" style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px'}}>
        <div onClick={() => setTransportMode("OWN_TRACTOR")} className={`p-4 border rounded-xl cursor-pointer flex items-center gap-3 transition-all ${transportMode === "OWN_TRACTOR" ? "border-green-600 bg-green-50 shadow-sm" : "border-gray-200 hover:border-gray-300"}`} style={{padding: '16px', border: transportMode === 'OWN_TRACTOR' ? '2px solid #16a34a' : '1px solid #e5e7eb', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: transportMode === 'OWN_TRACTOR' ? '#f0fdf4' : '#fff'}}>
          <input type="radio" name="transport_mode" value="OWN_TRACTOR" checked={transportMode === "OWN_TRACTOR"} onChange={() => setTransportMode("OWN_TRACTOR")} className="w-4 h-4 text-green-600" style={{accentColor: '#16a34a', width: '16px', height: '16px'}} />
          <div>
            <p className="font-medium text-gray-900 text-sm" style={{margin: 0, fontWeight: 500, fontSize: '0.875rem'}}>Apna Tractor / Commercial Gaadi</p>
            <p className="text-xs text-gray-500" style={{margin: 0, fontSize: '0.75rem', color: '#6b7280'}}>Instant gate-pass confirmed</p>
          </div>
        </div>
        <div onClick={() => setTransportMode("MANUAL_CART")} className={`p-4 border rounded-xl cursor-pointer flex items-center gap-3 transition-all ${transportMode === "MANUAL_CART" ? "border-green-600 bg-green-50 shadow-sm" : "border-gray-200 hover:border-gray-300"}`} style={{padding: '16px', border: transportMode === 'MANUAL_CART' ? '2px solid #16a34a' : '1px solid #e5e7eb', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: transportMode === 'MANUAL_CART' ? '#f0fdf4' : '#fff'}}>
          <input type="radio" name="transport_mode" value="MANUAL_CART" checked={transportMode === "MANUAL_CART"} onChange={() => setTransportMode("MANUAL_CART")} className="w-4 h-4 text-green-600" style={{accentColor: '#16a34a', width: '16px', height: '16px'}} />
          <div>
            <p className="font-medium text-gray-900 text-sm" style={{margin: 0, fontWeight: 500, fontSize: '0.875rem'}}>Cycle-Rickshaw / Thela / Jugad</p>
            <p className="text-xs text-gray-500" style={{margin: 0, fontSize: '0.75rem', color: '#6b7280'}}>No number plate required</p>
          </div>
        </div>
        <div onClick={() => setTransportMode("POOL")} className={`p-4 border rounded-xl cursor-pointer flex items-center gap-3 transition-all ${transportMode === "POOL" ? "border-green-600 bg-green-50 shadow-sm" : "border-gray-200 hover:border-gray-300"}`} style={{padding: '16px', border: transportMode === 'POOL' ? '2px solid #16a34a' : '1px solid #e5e7eb', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: transportMode === 'POOL' ? '#f0fdf4' : '#fff'}}>
          <input type="radio" name="transport_mode" value="POOL" checked={transportMode === "POOL"} onChange={() => setTransportMode("POOL")} className="w-4 h-4 text-green-600" style={{accentColor: '#16a34a', width: '16px', height: '16px'}} />
          <div>
            <p className="font-medium text-gray-900 text-sm" style={{margin: 0, fontWeight: 500, fontSize: '0.875rem'}}>Tractor Pooling Chahiye (Share)</p>
            <p className="text-xs text-gray-500" style={{margin: 0, fontSize: '0.75rem', color: '#6b7280'}}>Mandi / Cluster assign karegi</p>
          </div>
        </div>
      </div>
    </div>
    {transportMode === 'OWN_TRACTOR' && (
      <>
        <Field label="Vehicle number" required value={vehicleNumber} onChange={e => { if(e && e.target) setVehicleNumber(e.target.value.toUpperCase()) }} placeholder="BR01AB1234" />
        <Field label="Vehicle type" options={[{ value: 'TRACTOR_TROLLEY', label: 'Tractor trolley' }, { value: 'COMMERCIAL_TRUCK', label: 'Commercial truck' }]} value={vehicleType} onChange={e => { if(e && e.target) setVehicleType(e.target.value) }} />
        {vehicleType === 'COMMERCIAL_TRUCK' && <Field label="Transit permit" required value={transitPermit} onChange={e => { if(e && e.target) setTransitPermit(e.target.value) }} />}
      </>
    )}
    {transportMode === 'MANUAL_CART' && (
      <div style={{ gridColumn: '1 / -1', padding: '10px 14px', backgroundColor: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe', color: '#1e40af', fontSize: '0.85rem', marginTop: '10px' }}>
        ℹ️ <b>Wahan: Manual Cart / Rickshaw.</b> Gate aur weighbridge par entry QR code token se identify hogi.
      </div>
    )}
    {transportMode === 'POOL' && (
      <div style={{ gridColumn: '1 / -1' }}>
        <UberTractorPool quantity={quantity} onSelectDriver={setPoolDriverDetails} />
      </div>
    )}
  </>
)}</div><label className="check-field"><input type="checkbox" required /> I confirm that the details are correct and will bring the required documents.</label><div className="button-row"><Button secondary disabled={submitting} onClick={() => setStep(1)}>Back</Button><Button type="submit" disabled={submitting}>{submitting ? 'Saving…' : reschedule ? 'Confirm Reschedule' : 'Confirm & Book'}</Button></div></Card>}</form></>}{reschedule && <Button secondary onClick={() => {
      setReschedule(false);
      setStep(0);
