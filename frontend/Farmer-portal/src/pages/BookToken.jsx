...
export default function BookToken() {
  ...
  let active = null;
  if (activeBooking && !['Cancelled', 'Completed'].includes(activeBooking.status)) {
    active = {
      ...activeBooking,
      id: activeBooking.token_id,
      center: activeBooking.mandi_name || activeBooking.intended_mandi_id || data.centers[0].name,
      date: activeBooking.slot_time ? activeBooking.slot_time.split('T')[0] : dateISO(),
      slot: activeBooking.slot_time ? new Date(activeBooking.slot_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '09:00 AM',
      crop: activeBooking.crop_name,
      quantity: activeBooking.quantity_quintal,
  ...
  async function book(e) {
    ...
    try {
      ...
      const [timeStr, ampm] = cleanTime.split(' ');
      let [hours, minutes] = timeStr.split(':');
      hours = parseInt(hours, 10);
      if (ampm === 'PM' && hours < 12) hours += 12;
      if (ampm === 'AM' && hours === 12) hours = 0;
      
      const isoTime = new Date(`${date}T${String(hours).padStart(2, '0')}:${minutes}:00`).toISOString();

      const payload = {
          phone_number: user.phone_number,
          crop_name: crop,
          vehicle_number: "BR-01-XX-1234",
          vehicle_type: "TRACTOR",
          transit_permit: null,
          quantity_quintal: Number(quantity),
          slot_time: isoTime,
          intended_mandi_id: selected?.id || "SPC1234"
      };
      ...
      const completeBooking = {
        ...
        channel: response.channel || "FARMER_PORTAL",
        slot_time: response.slot_time || payload.slot_time,
        qr_image: response.qr_image,
        status: response.status || "CONFIRMED",
        pool_id: response.pool_id || null,
        queue_position: 1,
        estimated_wait_mins: 15
      };
      
      updateBookingState(completeBooking);
      
      notify('Token Generated', `Your token is confirmed at ${center} on ${date}, ${slot}.`);
      setSuccess(completeBooking);
      setReschedule(false);
      setStep(0);
  ...
  return <div className="page">{active && !reschedule ? <Card className="current-booking"><div><Badge>{active.status}</Badge><h2>My current token · {active.id}</h2><p>{active.center} · {active.date} · {active.slot}</p><p>{active.quantity} Quintal {active.crop}</p></div><div className="button-row"><Button secondary onClick={() => download('token', active)}>Download Token</Button><Button onClick={() => {
      ...
      setStep(0);
          ...
          if (activeBooking && (activeBooking.token_id === active.id || activeBooking.id === active.id)) {
             updateBookingState({ ...activeBooking, status: 'Cancelled' });
          }
